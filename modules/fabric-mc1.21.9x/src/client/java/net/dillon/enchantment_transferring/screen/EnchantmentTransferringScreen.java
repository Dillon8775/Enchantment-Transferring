...
public class EnchantmentTransferringScreen extends ForgingScreen<EnchantmentTransferrerScreenHandler> {
    ...
    public EnchantmentTransferringScreen(EnchantmentTransferrerScreenHandler handler, PlayerInventory inventory, Text title) {
        ...
        if (this.focusedSlot != null) {
            ...
            if (focusedSlotStack.isEmpty()) {
                switch (this.focusedSlot.id) {
                    case 0 -> optional = Optional.of(Text.translatable("block.enchantment_transferrer.enchantment_transferrer.enchanted_tool").formatted(Formatting.AQUA));
                    case 1 -> optional = Optional.of(Text.translatable("block.enchantment_transferrer.enchantment_transferrer.unenchanted_tool").formatted(Formatting.LIGHT_PURPLE));
                }
            }
        }
    ...
    protected void drawForeground(DrawContext context, int mouseX, int mouseY) {
        ...
        if (i > 0) {
            ...
            if (!this.handler.getSlot(2).hasStack()) {
                ...
                text = Text.translatable("block.enchantment_transferrer.enchantment_transferrer.cost", i);
